...
class BaselinePatchNet(nn.Module):
    def __init__(self, d_model, n_streams, s_len, depth=3, n_heads=4):
        super().__init__()
        self.n, self.s, self.d = n_streams, s_len, d_model
        self.layers = nn.ModuleList([StandardTransformerBlock(d_model, n_heads=n_heads) for _ in range(depth)])
...
class mHCPatchNet(nn.Module):
    def __init__(self, d_model, n_streams, s_len, depth=3, n_heads=4, evaluator=None):
        ...
        self.layers = nn.ModuleList([
            mHC_StandardTransformerBlock(d_model, n_heads, n_streams, s_len,evaluator=evaluator)
...
class TinyVLM(nn.Module):
    def __init__(self, d_model, n_streams, s_len, depth=3, n_heads=4,evaluator=None,mHC=False):
        super().__init__()
        if mHC: self.backbone = BaselinePatchNet(d_model, n_streams, s_len, depth, n_heads)
        else: self.backbone = mHCPatchNet(d_model, n_streams, s_len, depth, n_heads,evaluator=evaluator)
...
def plot_mhc_analysis(evaluator, base_loss_history, mhc_loss_history, base_grad_norms, time_base, time_mhc,
    ...
    # --- 1. 左上：梯度范数随步数变化 (Grad Norm Stability) ---
    ax1 = axes[0, 0]
    ...
    # --- 2. 右上：信号传播动力学 (对应论文 Figure a: Amax Gain vs Layer) ---
    ...
    plt.suptitle(f"mHC Unrolled Transformer vs Baseline: VLM Test (Depth={depth}, n_streams={n_streams})",
                 fontsize=16, fontweight='bold', y=0.98)
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig("mHC_Final_Report.png", dpi=300)
    plt.show()
...
if __name__ == "__main__":
    torch.manual_seed(0)
    B, dim, n_streams, H, W = 16, 16, 4, 128, 128
    patch_size = 8
    depth = 16  # 增加深度以观察梯度优势
    steps = 200
    lr = 1e-3
    num_patches = (H // patch_size) * (W // patch_size)
    s_len = num_patches // n_streams

    patchify = nn.Conv2d(dim, dim, kernel_size=patch_size, stride=patch_size)
    with torch.no_grad():
    ...
    baseline = BaselinePatchNet(dim, n_streams, s_len, depth)
    mhc = mHCPatchNet(dim, n_streams, s_len, depth, evaluator=evaluator)
    ...
    for step in tqdm(range(steps), desc="Training Baseline"):
        x, y = get_batch()
        out_base = baseline(x)
        loss_base = F.mse_loss(out_base, y)
        opt_base.zero_grad()
        loss_base.backward()
    ...
    # --- 测试 mHC 速度 ---
    start_time = time.perf_counter()
    for step in tqdm(range(steps), desc="Training mHC"):
        x, y = get_batch()
        x_mhc = x.clone().requires_grad_(True)
        out_mhc = mhc(x_mhc)
        loss_mhc = F.mse_loss(out_mhc, y)
        opt_mhc.zero_grad()
        loss_mhc.backward()
        opt_mhc.step()
    ...
    # --- 打印分析 ---
    print(f"Baseline Cost: {time_base:.2f}s (每步 {time_base / steps:.4f}s)")
    print(f"mHC Cost: {time_mhc:.2f}s (每步 {time_mhc / steps:.4f}s)")
    ...
    # 调用绘图
    plot_mhc_analysis(evaluator, base_loss_list, mhc_loss_list, base_avg_grad_history, time_base, time_mhc,steps, depth)